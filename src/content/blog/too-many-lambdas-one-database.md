...
The symptoms sat next to the [slow serializer problems we had elsewhere](/blog/britelines-cache-generation). Timeouts, failed jobs, a product whose cache never quite finished. It was easy to treat it as the same bug, but it was not. One path was too many queries inside a worker. This one was too many workers asking the database for a seat at the same moment.
...
The idea was simple. Keep some parallelism so a full reset does not drag forever, but cap how many workers hit MySQL at once so the connection limit stops being the bottleneck. We were not trying to make one job faster. We were trying to stop a healthy fan out from drowning the database.

#### **Why this is easy to miss**

Fan out feels like the cloud working as intended. You have a queue, you have Lambdas, you have a product with many pieces, so of course you process them together. The database does not scale with that for free. Connection limits are a hard ceiling, and retries turn a short spike into a longer outage of the same shape.

I have seen the same pattern outside BriteLines. People migrate a table with a swarm of workers, warm a cache for every tenant at deploy time, or fan out “just regenerate everything.” Each worker looks fine on its own, but together they exhaust a shared resource that is not in the Lambda metrics you stare at first.
...
We already had the split by risk type. The missing piece was seeing that the split could work too well. Once the workers stopped landing on MySQL all at once, full version resets stopped failing on connection limits.
